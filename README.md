...
#Out:
```
CREATE TABLE `test_table_local` (  `id` Int64  COMMENT '主键(自动递增)',  `user_id` Int64 COMMENT '用户id',  `status` Int8  COMMENT '用户名、账号',  `user_name` Nullable(String) COMMENT '用户名、账号',  `table_id` Nullable(String) COMMENT '当前的表名',  `filed` Nullable(String) COMMENT '存放有权限的字段',  `database_id` Nullable(String) COMMENT '主题数据库',  `is_del` Nullable(String) COMMENT '删除状态  ',  `created_date`  DateTime  COMMENT '创建时间',  `created_by` Nullable(String) COMMENT '创建人id',  `created_name` Nullable(String) COMMENT '创建人姓名',  `updated_date`  DateTime  COMMENT '更新时间',  `updated_by` Nullable(String) COMMENT '更新人id',  `updated_name` Nullable(String) COMMENT '更新人name',  `flag` Nullable(String) COMMENT '0:待审核  1:通过  2:未通过',  `apply_reason` String COMMENT '申请原因',  `ch_name` Nullable(String) COMMENT '表中文名',  `descri_table` Nullable(String) COMMENT '表描述',  `owner` Nullable(String) COMMENT '表的所有者',  `audit_advice` Nullable(String) COMMENT '审批意见',  `db_comment` Nullable(String) COMMENT '业务名称（数据库描述）',  `db_class` Nullable(String) COMMENT '业务分类',  `table_record_id` Nullable(String) COMMENT '表的记录Id') ENGINE = Memory 
